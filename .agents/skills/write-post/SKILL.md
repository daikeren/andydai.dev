...
3. 判斷 Andy 處於哪個階段，從那裡接手（見下）。**不要**每次都從 Interview 從頭跑。

## 他帶著什麼來，決定你從哪開始

- **只有一個種子 / 模糊念頭** → 進階段一 Interview。
- **已經有完整 brief 或講清楚的論點+故事** → 跳過 Interview，直接 outline + title 候選 + 編輯 flag，進階段二。
...
- **Exit criteria**：問到他自己講出 rough structure，才進下一階段。
- 如果他是「論點先行」但還沒有具體故事，標註 `[沒有 anchor story]`，但**不 block**——繼續往下。

## 階段二：Draft

- 把他講的整理成文章。**盡量保留他原本的措辭**，不要 sanitize、不要把 code-switching 或刻意的粗糙磨平。粗邊是故意的。
- **保持中性**：節奏和 punch 是他在 Review 時自己加的，Draft 階段**不主動調**。
