...
description: Anthropic 發表論文宣稱 Claude 有 171 種「功能性情緒」。我用 Claude 自己拆解這篇論文——steering 實驗有料，但方法論決定了結論，選題策略才是真正聰明的地方。
...
> **TL;DR**: Anthropic 說 Claude 有 171 種「功能性情緒」。技術不假——steering 實驗確實能改變模型行為。但方法論保證你一定找得到東西，而「情緒」這個框架是選題策略，不是科學必然。這是 research-grade PR：用學術形式做品牌定位。
...
但這也正是問題所在。論文用「情緒」來框架這個發現，暗示了一個從 representation 到 experience 的跳躍。如果他們把同一組發現框架成「contextual pressure vectors that influence decision-making under stress」— 技術上完全等價，但不會有人因此覺得 AI 有感情。
