...
**主動呼叫工具。** LLM 能自己去 query CloudWatch、自己讀 codebase。它有「手」了，不是只能坐在那邊等你餵資料。我們在 Codeer 自己寫了一個 agent skill，讓 Codex 可以查 AWS CloudWatch Logs。技術上不算神奇，就是把查詢 CloudWatch 的能力接成 agent skill。真正重要的不是 skill 本身，而是 AI 終於能自己補齊調查需要的上下文。
...
更大的改變不是少看幾分鐘 log，而是 incident handling 開始從高手手工藝，變成可以被系統化、被複用的流程。

下一次 incident 發生時，先不要只想著把 dashboard 打開得更快。先問自己：你的 AI 能不能自己查 log、讀 code、接上 incident context，先替你做完第一輪調查？

如果不能，你優化的還是監控；如果可以，才開始接近真正有用的 AI Ops。
